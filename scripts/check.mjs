...
const fabricAdapter = readFileSync(join(root, "reference", "fabric.md"), "utf8");
...
assert.match(adapter, /sole subagent runtime/);
assert.match(adapter, /stop instead of falling back to Fabric agents/);
assert.match(fabricAdapter, /does \*\*not\*\* own subagents/);
assert.match(fabricAdapter, /Never fall back to `agents\.run`/);
assert.match(fabricAdapter, /extensions\.workflow/);
assert.doesNotMatch(skill, /\/skill:fabric-guide/);
console.log("designing-dynamic-workflows: package and sole-subagent routing verified");
